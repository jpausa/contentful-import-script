...
// const buildContentfulAssetsPromises = async ({
//   externalData,
//   assetLabels,
//   localeCode,
//   contentfulClient,
// }: IAssetsImport) => {
//   logger.info("Building Contentful new assets object to be imported");
//   const createAssetsPromises: Promise<any>[] = [];
//   const assetsIds: string[] = [];
//   externalData.slice(0, 10).forEach((rawEntry) => {
//     const newAssetId = rawEntry["id"];
//     const newAssetName = `${newAssetId}.${assetLabels.contentType.slice(
//       assetLabels.contentType.indexOf("/") + 1
//     )}`;

//     let newAssetObject = {
//       title: {
//         [localeCode]: rawEntry[assetLabels.title],
//       },
//       file: {
//         [localeCode]: {
//           contentType: assetLabels.contentType,
//           fileName: newAssetName,
//           upload: rawEntry[assetLabels.upload],
//         },
//       },
//     };

//       assetsIds.push(newAssetId);

//     createAssetsPromises.push(
//       contentfulClient
//         .createAssetWithId(newAssetId, { fields: { ...newAssetObject } })
...
//   return {
//     createAssetsPromises,
//     assetsIds,
//   };
// };

// const createAssetsEntriesLinksPromises = async ({
//   assetsIds,
//   assetType,
//   localeCode,
//   contentfulClient,
// }: ICreateAssetsEntriesLinks) => {
//   logger.info("Stablishing assets and entries relationships");
//   const assetsEntriesLinksPromises: Promise<any>[] = [];
//   assetsIds.forEach(async (id) => {
//     let entry = await contentfulClient.getEntry(id);

//     entry.fields[assetType] = {
//       [localeCode]: entry?.fields?.assetType?.localeCode
//         ? [
//             ...entry.fields[assetType][localeCode],
//             {
//               sys: {
//                 type: "Link",
//                 linkType: "Asset",
...
//     assetsEntriesLinksPromises.push(entry.update());
//   });

//   return assetsEntriesLinksPromises;
// };
...
// const importAssetsToContentful = async ({
//   externalData,
//   assetLabels,
//   localeCode,
//   contentfulClient,
// }: IAssetsImport) => {
//   const { createAssetsPromises, assetsIds } =
//     await buildContentfulAssetsPromises({
//       externalData,
//       assetLabels,
//       localeCode,
//       contentfulClient,
...
//   const assetsImportingResults = await processDataRequests(
//     createAssetsPromises
//   );

//     const assetType = `${assetLabels.contentType.slice(
//       0,
//       assetLabels.contentType.indexOf("/")
//     )}`;

//     const assetsEntriesLinksPromises = await createAssetsEntriesLinksPromises({
//       assetsIds,
//       assetType,
//       localeCode,
//       contentfulClient,
...
//     await processDataRequests(assetsEntriesLinksPromises);

//   let rejectedAssetsImports: any[] = [];
//   assetsImportingResults.forEach((entry) => {
//     if (entry.status === "rejected") {
//       const errorObject = JSON.parse(entry?.reason?.message);
//       rejectedAssetsImports.push({
//         errorStatus: errorObject?.status, // If error status code is 409. From https://www.contentful.com/developers/docs/references/errors/  -> This error occurs when you're trying to update an existing asset, entry or content type, and you didn't specify the current version of the object or specified an outdated version.
//         errorMessage: errorObject?.statusText,
//         message: errorObject?.message,
//         details: errorObject?.details,
...
//   });
//   logger.info(
//     `Job Completed: Assets import. Sucessfully ${
//       assetsImportingResults.length - rejectedAssetsImports.length
//     }. Failed: ${rejectedAssetsImports.length}. `,
//     rejectedAssetsImports.length > 0
//       ? {
//           rejectedImports: rejectedAssetsImports,
//         }
//       : "No errors found"
//   );
// };
